...
checkBashVersion() {
    if (( "${BASH_VERSION%%.*}" < 4 )); then
        ...
        if [[ ${OSTYPE:-} =~ [Dd]arwin ]]; then
            ...
            echo '"$(brew --prefix bash)/bin/bash" ./minizt.bash ...'
        fi
        exit 1;
    fi
}
...
BANNER
    ...
    if [[ -n "$profile" && "$profile" != "minizt" ]]; then
        echo -e "   .: $* :.\n"
    fi
}
...
_usage(){
    ...
    echo -e "\n Basic Commands:\n"\
            "   start\t\tstart minizt\n"\
            "   delete\t\tdelete minizt\n"\
            "   console\t\topen zt-console in browser\n"\
            "   creds\t\tprints admin user updb credentials\n"\
            "   status\t\tprint zt component status\n"\
            "   zt\t\tzt cli wrapper\n"\
            ...
            "\n Advanced Commands:\n"\
            "   shell\t\trun interactive shell inside the zt-controller container\n"\
            "   login\t\trun local zt binary edge login\n"\
            "\n Other Commands:\n"\
            ...
            "   --profile\t\tMINIKUBE_PROFILE (minizt)\n"\
            ...
            "   --values-dir\tEXTRA_VALUES_DIR with Helm values files named zt-controller.yaml or zt-router.yaml\n"\
            "   --devel\t\tinclude Helm chart prereleases (passes --devel to Hanzo ZT chart installs/upgrades)\n"\
            "   --now\t\teliminate safety waits, e.g., before deleting minizt\n"\
            "   --\t\t\tMINIKUBE_START_ARGS args after -- passed to minikube start\n"
}

_usageProfileUse(){
    echo -e "\nSet minizt default profile\n"\
            "\n Usage:\n"\
            "   zt profile use [profile]\n"
}
...
_usageProfile(){
    ...
    echo -e "\nProfile management commands\n"\
            ...
            "   use\t\tset minizt profile\n"
}

checkDns(){
    # only param is an IPv4
    [[ $# -eq 1 && $1 =~ ([0-9]{1,3}\.){3}[0-9]{1,3} ]] || {
        logError "need IPv4 of minizt ingress as only param to checkDns()"
        return 1
    }
    logDebug "checking dns to ensure ${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE} resolves to $1"
    if grep -qE "^${1//./\\.}.*${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}" /etc/hosts \
        || nslookup "${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}" | grep -q "${1//./\\.}"; then
        logDebug "host dns found expected minikube ingress IP '$1'"
        return 0
    else
        logError "${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE} does not resolve to '$1'. Did you add the record in /etc/hosts?"
        return 1
    fi
}
...
deleteMinizt(){
    ...
    if [[ $1 =~ ^[0-9]+$ ]]; then
        ...
        logDebug "no integer param detected to deleteMinizt(), using default wait time ${WAIT}s"
    fi
    (( SAFETY_WAIT )) && {
...
makeMiniztStateDir() {
    case "$DETECTED_OS" in
        macOS)
            state_dir="${XDG_STATE_DIR:-$HOME/Library/Application Support}/minizt"
            ;;
        Linux|Windows)
            state_dir="${XDG_STATE_DIR:-$HOME/.local/state}/minizt"
            ;;
        *)
    ...
    if [[ ! -d "$state_dir" ]]; then
        logDebug "Creating minizt state directory: $state_dir"
        mkdir -p "$state_dir"
    fi
...
testClusterDns(){
    if kubectlWrapper run "dnstest" --rm --tty --stdin --image busybox --restart Never -- \
        nslookup "${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}" | grep "$1" >&3; then
...
cleanHosts() {
    local marker="$1"
    logWarn "Removing stale minizt entries from $HOSTS_FILE"
    if (( EUID != 0 )); then
...
installHosts() {
    hosts=(
        "${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}"
    ...
    if ! grep -q "$hosts_line" "$HOSTS_FILE"; then
        ...
        logInfo "Adding minizt entries to $HOSTS_FILE"
        if (( EUID != 0 )); then
...
getAdminSecret() {
    kubectlWrapper get secrets "zt-controller-admin-secret" \
...
miniztLogin() {
    ...
    getAdminSecret | xargs zt edge login "https://${ZITI_NETWORK_NAME}.${ingress_zone}:443/edge/management/v1" \
            --cli-identity "$MINIKUBE_PROFILE" \
            --yes \
            --username "admin" \
            --password

    logInfo "Setting default zt identity to: $MINIKUBE_PROFILE"
    zt edge use "$MINIKUBE_PROFILE" >&3
}

miniztConsole() {
    ingress_zone="$(getIngressZone)"
    console_url="https://${ZITI_NETWORK_NAME}.${ingress_zone}/zac/"
    case "$DETECTED_OS" in
        "Windows")
    ...
    logInfo "Opening zt-console: $console_url ..."
}
...
controllerPod() {
    kubectlWrapper get pods \
        --selector app.kubernetes.io/component=zt-controller \
...
showStatus() {
    COMPONENTS=("zt-controller" "zt-router")
...
ztWrapper() {
    kubectlWrapper exec "$(controllerPod)" --container zt-controller -- ztLogin > /dev/null
    kubectlWrapper exec "$(controllerPod)" --container zt-controller -- zt "$@"
}

shellWrapper() {
    kubectlWrapper exec "$(controllerPod)" --container zt-controller --tty --stdin -- bash
}
...
main(){
    ...
    : "${ZITI_NETWORK_NAME:=minizt-controller}"
    MINIZITI_CONFIGMAP="minizt-config"
    ZITI_CLI_HOME="$(getZitiCliHome)"
    ZITI_CLI_CERTS_DIR="$ZITI_CLI_HOME/certs"
    STATE_DIR="$(makeMiniztStateDir)"
    ...
    if (( DELETE_MINIZITI )); then
        ...
        if ingress_zone="$(getIngressZone)"; then
            ...
            CERT_FILE="$(find "$ZITI_CLI_CERTS_DIR" -maxdepth 1 -type f -name "${ZITI_NETWORK_NAME}.$ingress_zone" -print -quit 2> /dev/null)"
            if [[ -n "$CERT_FILE" ]]; then
                logWarn "Deleting minizt certificate file: $CERT_FILE"
                rm -f  "$CERT_FILE"
            fi
        fi
        trap - EXIT
        ...
        if [[ -d "$PROFILE_DIR" ]]; then
            logWarn "Deleting minizt profile directory: $PROFILE_DIR"
            rm -rf  "$PROFILE_DIR"
        fi
        ...
        # Cannot nicely call logout until https://github.com/hanzozt/zt/issues/1305 is addressed.
        # if checkCommand zt &>/dev/null; then
        #     logWarn "Removing $MINIKUBE_PROFILE profile identity from zt-cli.json"
        #     zt edge logout --cli-identity "$MINIKUBE_PROFILE" >&3
        # fi
    ...
    if [[ ! -d "$IDENTITIES_DIR" ]]; then
        logDebug "Creating minizt identities directory: ($IDENTITIES_DIR)"
...
EOF
    ...
    if (( ZITI_CHARTS_ALT )) && [[ -s "${ZITI_CHARTS_REF}/zt-controller/Chart.lock" ]]; then
        helmWrapper dependency build "${ZITI_CHARTS_REF}/zt-controller" >&3
    fi
    local -a _controller_cmd=(upgrade --install "zt-controller" "${ZITI_CHARTS_REF}/zt-controller"
    ...
    local -a _controller_cmd=(upgrade --install "zt-controller" "${ZITI_CHARTS_REF}/zt-controller"
        --namespace "${ZITI_NAMESPACE}" --create-namespace
        --set clientApi.advertisedHost="${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}"
        --values "${ZITI_CHARTS_URL}/zt-controller/values-traefik-ingressroutetcp.yaml"
    )
    if (( HELM_INCLUDE_DEVEL )) && (( ! ZITI_CHARTS_ALT )); then
        _controller_cmd+=(--devel)
    fi
    if [[ -n "${EXTRA_VALUES_DIR:-}" && -s "${EXTRA_VALUES_DIR}/zt-controller.yaml" ]]; then
        _controller_cmd+=(--values "${EXTRA_VALUES_DIR}/zt-controller.yaml")
    fi
    helmWrapper "${_controller_cmd[@]}" >&3

    local web_identity_cert_name
    web_identity_cert_name="zt-controller-web-identity-cert"
    ...
    logInfo "restarting zt-controller deployment to reload reissued certificates"
    kubectlWrapper rollout restart deployment/zt-controller \
    ...
    logInfo "waiting for zt-controller to be ready"
    kubectlWrapper wait deployments zt-controller \
            --namespace "${ZITI_NAMESPACE}" \
            --for condition=Available=True \
            --timeout "${MINIZITI_TIMEOUT_SECS}s" >&3
    ...
    #
    ## Ensure Cluster DNS is Resolving ${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}
    #

    if ! testClusterDns "${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}" 2>/dev/null; then
...
EOF
    ...
    logInfo "Setting default zt identity to: $MINIKUBE_PROFILE"
    ztWrapper edge use "$MINIKUBE_PROFILE" >&3

    ROUTER_NAME='minizt-router'
    ROUTER_OTT="$IDENTITIES_DIR/$ROUTER_NAME.jwt"
    if  ztWrapper edge list edge-routers "name=\"$ROUTER_NAME\"" \
        | grep -q minizt-router; then
        logDebug "updating $ROUTER_NAME"
        ztWrapper edge update edge-router "$ROUTER_NAME" \
            --role-attributes "public-routers" >&3
    else
        logDebug "creating $ROUTER_NAME"
        ztWrapper edge create edge-router "$ROUTER_NAME" \
            --role-attributes "public-routers" \
            --tunneler-enabled >&3
        ztWrapper edge list edge-routers \
            "name=\"$ROUTER_NAME\"" \
            --output-json \
            | jq --exit-status --raw-output '.data[0].enrollmentJwt' > "$ROUTER_OTT"
    fi

    logDebug "installing router chart as 'zt-router'"
    if (( ZITI_CHARTS_ALT )) && [[ -s "${ZITI_CHARTS_REF}/zt-router/Chart.lock" ]]; then
        helmWrapper dependency build "${ZITI_CHARTS_REF}/zt-router" >&3
    fi
    local -a _router_cmd=(upgrade --install "zt-router" "${ZITI_CHARTS_REF}/zt-router"
        --namespace "${ZITI_NAMESPACE}"
        --set-file enrollmentJwt="$ROUTER_OTT"
        --set edge.advertisedHost="minizt-router.${MINIZITI_INGRESS_ZONE}"
        --set ctrl.endpoint="${ZITI_NETWORK_NAME}.${MINIZITI_INGRESS_ZONE}:443"
        --set "tunnel.mode=host"
        --values "${ZITI_CHARTS_URL}/zt-router/values-traefik-ingressroutetcp.yaml"
    ...
    if [[ -n "${EXTRA_VALUES_DIR:-}" && -s "${EXTRA_VALUES_DIR}/zt-router.yaml" ]]; then
        _router_cmd+=(--values "${EXTRA_VALUES_DIR}/zt-router.yaml")
    fi
    helmWrapper "${_router_cmd[@]}" >&3

    logInfo "waiting for zt-router to be ready"
    kubectlWrapper wait deployments "zt-router" \
        --namespace "${ZITI_NAMESPACE}" \
        --for condition=Available=True \
        --timeout "${MINIZITI_TIMEOUT_SECS}s" >&3

    logDebug "probing minizt-router for online status"
    if ztWrapper edge list edge-routers "name=\"$ROUTER_NAME\"" \
        | awk '/minizt-router/ {print $6}' \
        | grep -q true; then
        logInfo "minizt-router is online"
    else
        logError "minizt-router is offline"
        exit 1
    fi
    ...
    if  ! ztWrapper edge list identities "name=\"$CLIENT_NAME\"" --csv \
        | grep -q "$CLIENT_NAME"; then
        logDebug "creating identity $CLIENT_NAME"
        ztWrapper edge create identity "$CLIENT_NAME" \
            --role-attributes httpbin-clients >&3
        ztWrapper edge list identities "name=\"$CLIENT_NAME\"" \
            --output-json \
    ...
    if  ! ztWrapper edge list identities "name=\"$HTTPBIN_NAME\"" --csv \
        | grep -q "$HTTPBIN_NAME"; then
        logDebug "creating identity $HTTPBIN_NAME"
        ztWrapper edge create identity "$HTTPBIN_NAME" \
            --role-attributes httpbin-hosts >&3
        ztWrapper edge list identities "name=\"$HTTPBIN_NAME\"" \
            --output-json \
    ...
    if  ! ztWrapper edge list configs 'name="httpbin-intercept-config"' --csv \
        | grep -q "httpbin-intercept-config"; then
        logDebug "creating config httpbin-intercept-config"
        ztWrapper edge create config "httpbin-intercept-config" intercept.v1 \
    ...
    if [[ -s "$HTTPBIN_OTT" ]]; then
        logDebug "installing httpbin chart as 'minizt-httpbin'"
        _httpbin_cmd=(
            install "minizt-httpbin" "${ZITI_CHARTS_REF}/httpbin"
            --set-file ztEnrollment="$HTTPBIN_OTT"
            --set ztServiceName=httpbin-service
        )
        if (( HELM_INCLUDE_DEVEL )) && (( ! ZITI_CHARTS_ALT )); then
            _httpbin_cmd+=(--devel)
        fi
        ...
        rm -f "$HTTPBIN_OTT"
        logDebug "deleted $HTTPBIN_OTT after installing successfully with minizt-httpbin chart"
    else
        logDebug "upgrading httpbin chart as 'minizt-httpbin'"
        _httpbin_cmd=(
            upgrade "minizt-httpbin" "${ZITI_CHARTS_REF}/httpbin"
            --set ztServiceName=httpbin-service
        )
        if (( HELM_INCLUDE_DEVEL )) && (( ! ZITI_CHARTS_ALT )); then
            _httpbin_cmd+=(--devel)

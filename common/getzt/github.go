...
func GetLatestGitHubReleaseVersion(org, ztApp string, verbose bool) (semver.Version, error) {
	var result semver.Version
	release, err := GetHighestVersionGitHubReleaseInfo(org, ztApp, verbose)
	if release != nil {
...
func GetLatestGitHubReleaseAsset(org, appName string, appGitHub string, version string, verbose bool) (*GitHubReleasesData, error) {
	if version != "latest" {
		if appName == "zt-prox-c" {
			version = strings.TrimPrefix(version, "v")
		}

		if appName == "zt" || appName == "zt-edge-tunnel" || appName == "zrok" || appName == "caddy" {
...
func FindVersionAndInstallGitHubRelease(org, app string, ztAppGitHub string, targetOS, targetArch string, binDir string, version string, verbose bool) error {
	...
	release, err := GetLatestGitHubReleaseAsset(org, app, ztAppGitHub, releaseVersion, verbose)
	if err != nil {
		return err
	}
...
func InstallGitHubRelease(ztApp string, release *GitHubReleasesData, binDir string, targetOS, targetArch, version string) error {
	releaseUrl, err := release.GetDownloadUrl(ztApp, targetOS, targetArch)
	if err != nil {
		return err
	}
	...
	if strings.HasSuffix(fileName, ".zip") {
		if ztApp == c.ZT_EDGE_TUNNEL {
			...
			err = Unzip(fullPath, binDir, func(path string) (string, bool) {
				if path == "zt-edge-tunnel" {
					count++
					return ztFileName, true
				}
				return "", false
			})
			...
			if count != 1 {
				return errors.Errorf("didn't find zt-edge-tunnel executable in release archive. count: %v", count)
			}

			pfxlog.Logger().Infof("Successfully installed '%s' version '%s' to %s", ztApp, release.Version, filepath.Join(binDir, ztFileName))
			return nil
		} else {
		...
		if ztApp == c.ZT {
			...
			if count != 1 {
				return errors.Errorf("didn't find zt executable in release archive. count: %v", count)
			}

			pfxlog.Logger().Infof("Successfully installed '%s' version '%s' to %s", ztApp, release.Version, filepath.Join(binDir, ztFileName))
			return nil
		} else if ztApp == c.ZROK {
			count := 0
			ztFileName := "zrok-" + version
			expectedPath := "zrok"
			...
			pfxlog.Logger().Infof("Successfully installed '%s' version '%s' to %s", ztApp, release.Version, filepath.Join(binDir, ztFileName))
			return nil
		} else if ztApp == c.Caddy {
			count := 0
			ztFileName := "caddy-" + version
			expectedPath := "caddy"

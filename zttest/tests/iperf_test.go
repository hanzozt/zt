...
import (
	...
	"github.com/hanzozt/zt/zttest/models/smoke"
	"github.com/stretchr/testify/require"
)

func TestIPerf(t *testing.T) {
	waitForTerminators(t, 30*time.Second,
		"ert-iperf", "ert-iperf-unencrypted",
		"zt-tunnel-iperf", "zt-tunnel-iperf-unencrypted")

	t.Run("iperf-tests", func(t *testing.T) {
		t.Run("ert-hosted", func(t *testing.T) {
			t.Parallel()

			for _, clientType := range []string{"ert" /*"zet",*/, "zt-tunnel"} {
		...
		//	t.Parallel()
		//
		//	for _, clientType := range []string{"zet", "zt-tunnel", "ert"} {
		//		for _, encrypted := range []bool{true, false} {
		//			for _, reversed := range []bool{true, false} {
		...
		t.Run("zt-tunnel-hosted", func(t *testing.T) {
			t.Parallel()

			for _, clientType := range []string{"zt-tunnel", "ert" /*, "zet"*/} {
				for _, encrypted := range []bool{true, false} {
					for _, reversed := range []bool{true, false} {
						testIPerf(t, clientType, "zt-tunnel", encrypted, reversed)
					}
				}
			}
		})

...
func TestDownloadFiles(t *testing.T) {
	...
	waitForTerminators(t, 30*time.Second,
		...
		"zt-tunnel-files", "zt-tunnel-files-unencrypted")
	...
	t.Run("download-tests", func(t *testing.T) {
		...
		t.Run("test-zt-tunnel-downloads", func(t *testing.T) {
			t.Parallel()
			...
			for _, size := range smoke.FileSizes {
				for _, hostType := range []string{"zt-tunnel", "ert", "zet"} {
					for _, client := range smoke.HttpClients {
						for _, encrypted := range []bool{true, false} {
							success := testFileDownload(t, "zt-tunnel", client, hostType, encrypted, size)
							checkZetHost(hostType, success)
						}
					}
				}

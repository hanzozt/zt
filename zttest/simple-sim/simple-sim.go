...
func newSimpleSimCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Generates traffic to one more zt identities",
...
func (self *simpleSimAction) startSimForIdentity(path string) {
	...
	ztConfig, err := zt.NewConfigFromFile(path)
	if err != nil {
		log.WithError(err).Fatal("unable to load zt identity")
		return
	}

	ztContext, err := zt.NewContext(ztConfig)
	if err != nil {
		log.WithError(err).Fatal("could not create sdk context from config")
	}

	go self.runSim(path, ztContext)
}

func (self *simpleSimAction) runSim(path string, ctx zt.Context) {
	log := pfxlog.Logger().WithField("config", path)

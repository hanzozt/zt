...
func TestEdgeRouterCsrFields(t *testing.T) {
	...
	assert.Equal(t, "", data.Router.Edge.CsrST)
	assert.Equal(t, "", config1.Edge.Csr.Province)
	assert.Equal(t, "", data.Router.Edge.CsrL)
	assert.Equal(t, "", config1.Edge.Csr.Locality)
	assert.Equal(t, "Hanzo AI", data.Router.Edge.CsrO)
	assert.Equal(t, "Hanzo AI", config1.Edge.Csr.Organization)

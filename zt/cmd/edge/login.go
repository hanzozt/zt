...
import (
	...
	"github.com/hanzozt/zt/v2/internal/jwtutil"
...
func addLoginAnnotation(cmd *cobra.Command, flagName string) {
	_ = ztcobra.AddFlagAnnotation(cmd, flagName, LoginFlagKey, "true")
}
...
func AddLoginFlags(cmd *cobra.Command, options *LoginOptions) {
	...
	cmd.Flags().StringVarP(&options.ServiceName, "service", "s", "", "The service name to use. When set the file will be used to create a ztfied connection")
...
func (o *LoginOptions) Run() error {
	...
	if o.File != "" {
		cfg, err := zt.NewConfigFromFile(o.File)
		if err != nil {

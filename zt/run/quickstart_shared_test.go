...
import (
	...
	api_client_config "github.com/hanzozt/edge-api/rest_management_api_client/config"
...
func enrollIdentity(client *rest_management_api_client.ZitiEdgeManagement, identityID string) (*zt.Config, error) {
...
func createZitifiedHttpClient(idFile string) http.Client {
	...
	ztContext.(*zt.ContextImpl).CtrlClt.SetAllowOidcDynamicallyEnabled(false)
	...
	ztTransport.TLSClientConfig = &tls.Config{InsecureSkipVerify: true}
...
func performQuickstartTest(t *testing.T) {
	...
	log.Infof("connecting user: %s to %s", ztAdminUsername, ctrlAddress)
	...
	client, err := rest_util.NewEdgeManagementClientWithUpdb(ztAdminUsername, ztAdminPassword, ctrlAddress, caPool)
	if err != nil {
		t.Fatal(err)
	}

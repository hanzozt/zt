...
import (
	...
	"github.com/hanzozt/zt/v2/zt/cmd/helpers"
...
func (o *QuickstartOpts) run(ctx context.Context) error {
	...
	if o.IsHA {
		go func() {
			...
			fmt.Printf("  zt edge quickstart join \\\n")
...
func (o *QuickstartOpts) CreateMinimalPki() {
	...
	//zt pki create ca --pki-root="$pkiDir" --ca-file="root-ca" --ca-name="root-ca" --spiffe-id="whatever"
	if o.joinCommand {
	...
	//zt pki create intermediate --pki-root "$pkiDir" --ca-name "root-ca" --intermediate-name "intermediate-ca" --intermediate-file "intermediate-ca" --max-path-len "1" --spiffe-id="whatever"
...
func (o *QuickstartOpts) configureOverlay() {
	...
	// zt edge create edge-router-policy all-endpoints-public-routers --edge-router-roles "#public" --identity-roles "#all"
	...
	// zt edge create service-edge-router-policy all-routers-all-services --edge-router-roles "#all" --service-roles "#all"
